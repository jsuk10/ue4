...
public class IOSPlatform : Platform
{
	...
	public override void GetFilesToDeployOrStage(ProjectParams Params, DeploymentContext SC)
	{
		...
		{
			// Stage files from UEMetadata.  Currently, only the Privacy manifest is supported
			DirectoryReference PrivacyDataPath = DirectoryReference.Combine(SC.EngineRoot, "Build", PlatformName, "Resources", "UEMetadata");
			FileReference PrivacyManifestFile = FileReference.Combine(PrivacyDataPath, "PrivacyInfo.xcprivacy");
			if (FileReference.Exists(PrivacyManifestFile))
			{
