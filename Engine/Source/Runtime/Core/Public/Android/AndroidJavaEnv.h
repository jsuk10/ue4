...
namespace AndroidJavaEnvPrivate
{
	template <typename T>
	struct TJniScopedObjectDeleter
	{
		static void Delete(JNIEnv*, T) {};
	};

	template <>
	struct TJniScopedObjectDeleter<jobjectArray>
	{
		static void Delete(JNIEnv* Env, jobjectArray Object)
		{
			const jsize Length = Env->GetArrayLength(Object);
			for (jsize Idx = 0; Idx < Length; ++Idx)
			{
				jobject Element = Env->GetObjectArrayElement(Object, Idx);
...
public:
	...
	~FScopedJavaObject()
	{
		if (*this)
		{
			AndroidJavaEnvPrivate::TJniScopedObjectDeleter<T>::Delete(Env, ObjRef);
