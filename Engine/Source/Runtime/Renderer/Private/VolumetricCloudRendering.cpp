...
{
	auto CleanUpCloudDataPass = [&ViewsTmp = Views](FRDGBuilder& GraphBuilder)
	{
		AddPass(GraphBuilder, [&ViewsTmp](FRHICommandList&)
			{
				CleanUpCloudDataFunction(ViewsTmp);
			});
	};
...
void FSceneRenderer::RenderVolumetricCloudsInternal(FRDGBuilder& GraphBuilder, FCloudRenderContext& CloudRC)
{
	...
	else
	{
		...
		GraphBuilder.AddPass(
			...
			ERDGPassFlags::Raster,
			[RenderViewPassParameters, SceneTmp = Scene, &MainView, ViewUniformBuffer, bVisualizeConservativeDensityOrDebugSampleCount,
