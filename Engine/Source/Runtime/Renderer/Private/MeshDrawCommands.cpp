...
void FParallelMeshDrawCommandPass::WaitForTasksAndEmpty()
{
	...
	if (MaxNumDraws > 0)
	{
		...
		{
			FRHICommandListExecutor::GetImmediateCommandList().EnqueueLambda([PrimitiveIdVertexBufferPoolEntryTmp = PrimitiveIdVertexBufferPoolEntry](FRHICommandListImmediate&) {
				GPrimitiveIdVertexBufferPool.ReturnToFreeList(PrimitiveIdVertexBufferPoolEntryTmp);
			});
		}
		else
		{
	...
	if (ParallelCommandListSet)
	{
		if (TaskContext.bUseGPUScene)
		{
			...
			RHICommandList.EnqueueLambda([
				...
				PrimitiveIdVertexBufferPoolEntryTmp = PrimitiveIdVertexBufferPoolEntry](FRHICommandListImmediate& CmdList)
			{
