...
FActiveSound* FActiveSound::CreateVirtualCopy(const FActiveSound& InActiveSoundToCopy, FAudioDevice& InAudioDevice)
{
	...
	FAudioThread::RunCommandOnGameThread([AudioComponentIDTmp = ActiveSound->GetAudioComponentID()]()
	{
		if (UAudioComponent* AudioComponent = UAudioComponent::GetAudioComponentFromID(AudioComponentIDTmp))
		{
...
void FActiveSound::UpdateWaveInstances(TArray<FWaveInstance*> &InWaveInstances, const float DeltaTime)
{
	...
	if (bIsInitFade)
	{
		...
		{
			FAudioThread::RunCommandOnGameThread([AudioComponentIDTmp = GetAudioComponentID()]()
			{
				if (UAudioComponent* AudioComponent = UAudioComponent::GetAudioComponentFromID(AudioComponentIDTmp))
				{
