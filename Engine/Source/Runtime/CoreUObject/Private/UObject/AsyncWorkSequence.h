...
public:
	...
	auto Finalize(EAsyncExecution Execution, CallableType&& Callable)
	{
		...
		return Async(Execution, [WorkItemsTmp = MoveTemp(WorkItems), CallableTmp = MoveTemp(Callable)] () mutable
		{
			StateType State;

			for (WorkFunctionType& Work : WorkItemsTmp)
			{
