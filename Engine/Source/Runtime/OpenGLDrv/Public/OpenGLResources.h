...
public:
	...
	virtual ~TOpenGLResourceProxy()
	{
		...
		{
			RunOnGLRenderContextThread([GLResourceObjectTmp = GLResourceObject]()
			{
				GLResourceObjectTmp->Release();
			});
			GLResourceObject = nullptr;
		}
	}
...
public:
	...
	virtual ~TOpenGLBuffer()
	{
		...
		if (Resource != 0)
		{
			auto DeleteGLResources = [ResourceTmp=Resource, RealSizeTmp= RealSize, bStreamDrawTmp= (bool)bStreamDraw, LockBufferTmp = LockBuffer, bLockBufferWasAllocatedTmp=bLockBufferWasAllocated]()
			{
				VERIFY_GL_SCOPE();
				if (BaseType::OnDelete(ResourceTmp, RealSizeTmp, bStreamDrawTmp, 0))
				{
					FOpenGL::DeleteBuffers(1, &ResourceTmp);
				}
				if (LockBufferTmp != NULL)
				{
					if (bLockBufferWasAllocatedTmp)
					{
						FMemory::Free(LockBufferTmp);
					}
					else
					{
						UE_LOG(LogRHI,Warning,TEXT("Destroying TOpenGLBuffer without returning memory to the driver; possibly called RHIMapStagingSurface() but didn't call RHIUnmapStagingSurface()? Resource %u"), ResourceTmp);
					}
				}
			};
...
public:
	...
	virtual ~FOpenGLTextureBase()
	{
		...
		{
			RunOnGLRenderContextThread([EvictionParamsPtrTmp = MoveTemp(EvictionParamsPtr)]() {
