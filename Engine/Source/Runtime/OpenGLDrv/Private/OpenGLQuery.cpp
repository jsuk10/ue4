...
FOpenGLRenderQuery::~FOpenGLRenderQuery()
{
	...
	if (Resource && !bInvalidResource)
	{
		...
		else
		{
			...
			ALLOC_COMMAND_CL(RHICmdList, FRHICommandGLCommand)([ResourceTmp = Resource, ResourceContextTmp = ResourceContext]() {VERIFY_GL_SCOPE(); ReleaseResource(ResourceTmp, ResourceContextTmp); });
		}
	}
}
...
{
	RunOnGLRenderContextThread([ProxyTmp = Proxy]()
	{
		VERIFY_GL_SCOPE();
		delete ProxyTmp;
	});
