...
FOpenGLShaderResourceView::~FOpenGLShaderResourceView()
{
	...
	{
		RunOnGLRenderContextThread([OpenGLRHITmp = OpenGLRHI, ResourceTmp = Resource]()
		{
			VERIFY_GL_SCOPE();
			OpenGLRHITmp->InvalidateTextureResourceInCache(ResourceTmp);
			FOpenGL::DeleteTextures(1, &ResourceTmp);
		});
	}
}
