...
{
	check(LinkedProgram);
	RunOnGLRenderContextThread([LinkedProgramTmp = LinkedProgram]()
	{
		const bool bIsEvicted = GetOpenGLProgramsCache().IsUsingLRU() && GetOpenGLProgramsCache().IsEvicted(LinkedProgramTmp->Config.ProgramKey);
		if( !bIsEvicted )
		{
			FOpenGLLinkedProgram* Prog = StaticLastReleasedPrograms[StaticLastReleasedProgramsIndex];
			StaticLastReleasedPrograms[StaticLastReleasedProgramsIndex++] = LinkedProgramTmp;
			...
			{
				FDelayedEvictionContainer::Get().Add(LinkedProgramTmp);
			}

			OnProgramDeletion(LinkedProgramTmp->Program);
		}
	});
}
