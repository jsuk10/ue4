		...
		public:
			...
			void DoWork()
			{
				...
				if (EffectSharedPtr.IsValid())
				{
					...
					TUniqueFunction<void()> Command = [Algo = MoveTemp(ConvReverb), EffectSharedPtr, VersionDataTmp = VersionData] () mutable
					{
						EffectSharedPtr->SetConvolutionReverbIfCurrent(MoveTemp(Algo), VersionDataTmp);
					};
