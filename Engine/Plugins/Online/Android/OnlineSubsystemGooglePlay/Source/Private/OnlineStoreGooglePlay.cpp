...
JNI_METHOD void Java_com_epicgames_ue4_GooglePlayStoreHelper_NativeQueryComplete(JNIEnv* jenv, jobject /*Thiz*/, jint ResponseCode, jobjectArray ProductIDs, jobjectArray Titles, jobjectArray Descriptions, jobjectArray Prices, jlongArray PriceValuesRaw, jobjectArray CurrencyCodes)
{
	...
	if (auto OnlineSubGP = static_cast<FOnlineSubsystemGooglePlay* const>(IOnlineSubsystem::Get(GOOGLEPLAY_SUBSYSTEM)))
	{
		OnlineSubGP->ExecuteNextTick([OnlineSubGP, EGPResponse, Response = MoveTemp(ProvidedProductInformation)]()
		{
			...
			else if (TSharedPtr<FOnlineStoreGooglePlayV2> StoreV2Int = StaticCastSharedPtr<FOnlineStoreGooglePlayV2>(OnlineSubGP->GetStoreV2Interface()))
			{
				StoreV2Int->OnGooglePlayAvailableIAPQueryComplete(EGPResponse, Response);
			}
		});
	}
}
