...
const TCHAR* FGoogleTransactionData::SubscriptionReceiptMarker = TEXT("isSubscription");
...
{ 
	if ((InOfferIds.Num() != 0) && FOnlinePurchaseGooglePlay::IsSubscriptionProductId(InOfferIds[0]))
	{
		IsSubscription = true;
	}
}
...
FOnlinePurchaseGooglePlay::FOnlinePurchaseInProgressTransaction::FOnlinePurchaseInProgressTransaction(
	...
	, UserId(StaticCastSharedRef<const FUniqueNetIdGooglePlay>(InUserId.AsShared()))
...
void FOnlinePurchaseGooglePlay::Checkout(const FUniqueNetId& UserId, const FPurchaseCheckoutRequest& CheckoutRequest, const FOnPurchaseCheckoutComplete& Delegate)
{
	...
	if (IsAllowedToPurchase(UserId))
	{
		...
		if (NumOffers > 0)
		{
			...
			ProductIds.Reserve(CheckoutRequest.PurchaseOffers.Num());
			for (const FPurchaseCheckoutRequest::FPurchaseOfferEntry& Entry : CheckoutRequest.PurchaseOffers)
			{
				ProductIds.Emplace(Entry.OfferId);
			}
...
void FOnlinePurchaseGooglePlay::OnTransactionCompleteResponse(EGooglePlayBillingResponseCode InResponseCode, const FGoogleTransactionData& InTransactionData)
{
	...
	if(InProgressTransaction != nullptr)
	{
		bool bWasInProgressTransactionCanceled = (Result == EPurchaseTransactionState::Canceled && (InTransactionData.GetOfferIds().Num() == 0));
...
JNI_METHOD void Java_com_epicgames_ue4_GooglePlayStoreHelper_NativeQueryExistingPurchasesComplete(JNIEnv* jenv, jobject /*Thiz*/, jint ResponseCode, jobjectArray ProductIDs, jintArray PurchaseStatesArray, jobjectArray PurchaseTokens, jobjectArray ReceiptsData, jobjectArray Signatures)
{
	...
	if (auto OnlineSubGP = static_cast<FOnlineSubsystemGooglePlay* const>(IOnlineSubsystem::Get(GOOGLEPLAY_SUBSYSTEM)))
	{
		...
		{
			if (TSharedPtr<FOnlineStoreGooglePlay> StoreInt = StaticCastSharedPtr<FOnlineStoreGooglePlay>(OnlineSubGP->GetStoreInterface()))
			{
				StoreInt->OnRestorePurchasesComplete(EGPResponse, ExistingPurchaseInfo);
			}
			else if (TSharedPtr<FOnlinePurchaseGooglePlay> PurchaseInt = StaticCastSharedPtr<FOnlinePurchaseGooglePlay>(OnlineSubGP->GetPurchaseInterface()))
			{
...
JNI_METHOD void Java_com_epicgames_ue4_GooglePlayStoreHelper_NativePurchaseComplete(JNIEnv* jenv, jobject /*Thiz*/, jint JavaResponseCode, jobjectArray JavaProductIds, jint JavaPurchaseState, jstring JavaPurchaseToken, jstring JavaReceiptData, jstring JavaSignature)
{
	...
	if (auto OnlineSubGP = static_cast<FOnlineSubsystemGooglePlay* const>(IOnlineSubsystem::Get(GOOGLEPLAY_SUBSYSTEM)))
	{
		OnlineSubGP->ExecuteNextTick([OnlineSubGP, EGPResponse, TransactionData = MoveTemp(TransactionData)]()
		{
			if (TSharedPtr<FOnlineStoreGooglePlay> StoreInt = StaticCastSharedPtr<FOnlineStoreGooglePlay>(OnlineSubGP->GetStoreInterface()))
			{
				StoreInt->OnProcessPurchaseResult(EGPResponse, TransactionData);
			}
			else if (TSharedPtr<FOnlinePurchaseGooglePlay> PurchaseInt = StaticCastSharedPtr<FOnlinePurchaseGooglePlay>(OnlineSubGP->GetPurchaseInterface()))
			{
...
JNI_METHOD void Java_com_epicgames_ue4_GooglePlayStoreHelper_NativeConsumeComplete(JNIEnv* jenv, jobject /*Thiz*/, jint JavaResponseCode, jstring JavaPurchaseToken)
{
	FString PurchaseToken = FJavaHelper::FStringFromParam(jenv, JavaPurchaseToken);;
	EGooglePlayBillingResponseCode EGPResponse = (EGooglePlayBillingResponseCode)JavaResponseCode;
...
JNI_METHOD void Java_com_epicgames_ue4_GooglePlayStoreHelper_NativeAcknowledgeComplete(JNIEnv* jenv, jobject /*Thiz*/, jint JavaResponseCode, jstring JavaPurchaseToken)
{
	FString PurchaseToken = FJavaHelper::FStringFromParam(jenv, JavaPurchaseToken);;
	EGooglePlayBillingResponseCode EGPResponse = (EGooglePlayBillingResponseCode)JavaResponseCode;
