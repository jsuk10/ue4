	...
	FOnAchievementsWrittenDelegate Delegate)
{
	...
	for (const TMap<FName, FVariantData>::ElementType& PropertyEntry : WriteObject->Properties)
	{
		const FString& Key = PropertyEntry.Key.ToString();
		const FVariantData& Stat = PropertyEntry.Value;
